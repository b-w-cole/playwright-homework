...
test('Validate selected pet types from the list', async ({page}) => {
  ...
  await pm.onOwnersPage().clickOwnerNameFor(petOwner)
  ...
  await pm.onOwnerInformationPage().clickEditPetFor(petName)
  ...
  await pm.onEditPetPage().validateNameOwnerAndPetTypeFieldsDisplayCorrectly(petName, petOwner, petType)
  ...
  await pm.onEditPetPage().validateAllPetValuesFromDropdownDisplayedInPetTypeField()
...
test('Validate the pet type update', async ({page}) => {
  ...
  await pm.onEditPetPage().updatePetTypeTo(testPetType)

  /* 10. On the "Owner Information" page, add the assertion that pet "Rosy" has a new value of the Type "bird" */
  await pm.onOwnerInformationPage().validatePetType(petName, testPetType)
  ...
  await pm.onEditPetPage().validateNameOwnerAndPetTypeFieldsDisplayCorrectly(petName, petOwner, testPetType)

  await pm.onEditPetPage().updatePetTypeTo(petType)
